...
app = typer.Typer(
    ...
    help=(
        ...
        "  A whole CSV file of entities to match?      → screen input.csv -i col=prop …\n"
        ...
        "\n"
        "NOTE: any matching task uses `match` (or `screen` for files), even with partial input\n"
        "(name only, name+country, etc.). `search` is purely for human-typed\n"
        "search UIs and isn't a fallback for `match`.\n"
        "\n"
